...
# Δημιουργία του game_mode
game_mode_var = tk.StringVar()
game_mode_var.set("Player vs Player")

# Δημιουργία της επικεφαλίδας
header_label = tk.Label(root, text="Welcome to Tic Tac Toe\nChoose an option to begin", font=("Helvetica", 16),
                        pady=10, background='#66b3ff')
header_label.pack()

# Δημιουργία frame για να κρατάει τα κουμπιά.
...
human_vs_human_label = tk.Label(button_frame, text="Human Vs Human", font=("Consolas", 12, "bold"), bg='#66b3ff')
human_vs_human_label.grid(row=1, column=0)

human_vs_human_button = tk.Button(button_frame, image=human_vs_human_photo,
                                  command=lambda mode="Player vs Player": open_registration_window(mode, game_mode_var),
                                  activebackground="red", bg="#66b3ff", cursor="gumby", border=0)
human_vs_human_button.grid(row=0, column=0, padx=5, pady=5)

human_vs_pc_label = tk.Label(button_frame, text="Human Vs PC", font=("Consolas", 12, "bold"), bg='#66b3ff')
human_vs_pc_label.grid(row=1, column=1)

human_vs_pc_button = tk.Button(button_frame, image=human_vs_pc_photo,
                               command=lambda mode="Player vs Computer": open_registration_window(mode, game_mode_var),
                               activebackground="red", bg="#66b3ff", cursor="pirate", border=0)
human_vs_pc_button.grid(row=0, column=1, padx=5, pady=5)

pc_vs_pc_button = tk.Button(button_frame, image=pc_vs_pc_photo,
                            command=lambda mode="Computer vs Computer": open_registration_window(mode, game_mode_var),
                            activebackground="red", bg="#66b3ff", cursor="coffee_mug", border=0)
pc_vs_pc_button.grid(row=0, column=2, padx=5, pady=5)

pc_vs_pc_label = tk.Label(button_frame, text="PC Vs PC", font=("Consolas", 12, "bold"), background='#66b3ff')
pc_vs_pc_label.grid(row=1, column=2)

# Δημιουργία των κουμπιών Credits, Hall of Fame, Exit / οριζόντια τοποθέτηση
credits_button = tk.Button(button_frame, image=credits_photo, command=open_credits_window, activebackground="red",
                           bg="#66b3ff", cursor="hand2", border=0)
credits_button.grid(row=2, column=0, padx=5, pady=20)

hall_of_fame_button = tk.Button(button_frame, image=HallOfFame_photo, activebackground="red", bg="#66b3ff",
                                cursor="hand2", border=0, command=open_hall_of_fame)
hall_of_fame_button.grid(row=2, column=1, padx=5, pady=20)

exit_button = tk.Button(button_frame, image=Exit_photo, command=root.destroy, activebackground="red", bg="#66b3ff",
